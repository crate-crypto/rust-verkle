use ffi_interface::{get_tree_key_hash, Context, fr_from_le_bytes, deserialize_proof_query};
...
#[no_mangle]
pub extern "C" fn pedersen_hash(ctx: *mut Context, address: *const u8, tree_index_le: *const u8, out: *mut u8) {
    ...
    let hash = get_tree_key_hash(context, <[u8; 32]>::try_from(add).unwrap(), <[u8; 32]>::try_from(tree_index).unwrap());
...
#[no_mangle]
pub extern "C" fn multi_scalar_mul(ctx: *mut Context, scalars: *const u8, len: usize, out: *mut u8)
{
